    ...
    "environment": [
      {
        ...
        "value": "udacity-brad-azure-sql"
      },
      {
        "name": "AZURE_DOTNET_APP",
        "value": "udacity-brad-azure-dotnet-app"
      }
    ],
    "portMappings": [
      {
...
resource "aws_s3_bucket" "brads_bucket" {
  bucket = "brads-udacity-bucket"

  tags = {
    Name        = "Brads udacity bucket"
    Environment = "Dev"
  }
}

resource "aws_dynamodb_table" "brads-dynamodb-table" {
  name           = "GameScores"
